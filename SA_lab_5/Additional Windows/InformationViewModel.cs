...
namespace SA_lab_5.Additional_Windows
{
    public class InformationViewModel
    {
        ...
        private double DetermineLimit(Func<double, double> f, double target, double step)
        {
            double x = 0;
            while(Math.Abs(f(x) - target) >= 1E-6)
            {
                x += step;
            }
            return x * 1.2;
        ...
        public InformationViewModel(BaseCell cell)
        {
            ...
            FullnessModel.Series.Add(new FunctionSeries(fullnessFunc, 0, DetermineLimit(fullnessFunc, 1, 0.1), 0.01)
            { Color = OxyColors.Red });
            ...
            this.TimelinessModel.Series.Add(new FunctionSeries(timelinessFunc, 0, DetermineLimit(timelinessFunc, 0, 1), 0.1)
